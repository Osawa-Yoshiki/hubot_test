...
module.exports = (robot) ->
  ...
  robot.respond /状況を#(.*)に配信して/i, (msg) ->
    request = msg.http("https://slack.com/api/chat.postMessage?token=#{slack_token}&icon_url=https://goo.gl/udYNSb&channel=%23" + #{msg.match[1]} + "&username=motionbot&text=見込みが減少しました。&as_user=false")
                          .get()
    request (err, res, body) ->
      json = JSON.parse body
      msg.send "承知しました"